...
root.config(bg='red')
conn = sqlite3.connect('code.db') #To connect to sqlite3
c = conn.cursor() #To get a cursor 
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='myself' ''') #To check whether myself table exist then return 1 or 0
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
	c.execute("""CREATE TABLE myself (
        first_name text,
        last_name text,
        phnum integer,
        mail text
...
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='cf' ''')
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
...
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='hackerearth' ''') 
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
	c.execute("""CREATE TABLE hackerearth (
        earth_id text,
        earth_rat integer
        )""")       
}
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='hackerrank' ''') 
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
	c.execute("""CREATE TABLE hackerrank (
        rank_id text,
        rank_rat integer
        )""")       
}
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='cc' ''') 
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
...
c.execute(''' SELECT count(name) FROM sqlite_master WHERE type='table' AND name='atco' ''') 
if c.fetchone()[0]!=1 : { #IF count!=1 will create a new table
	c.execute("""CREATE TABLE atco (
        atco_id text,
        atco_rat integer
        )""")       
}
...
def edit():
    ...
    c.execute("""UPDATE hackerearth SET
        earth_id= :earth_id,
        earth_rat= :earth_rat
        WHERE oid= :oid""",
    ...
    c.execute("""UPDATE hackerrank SET
        rank_id= :rank_id,
        rank_rat= :rank_rat
        WHERE oid= :oid""",
...
def update_now():
    ...
    c.execute("SELECT * FROM myself WHERE oid=" + record_id)
    records=c.fetchall()
    c.execute("SELECT * FROM cf WHERE oid=" + record_id)
    cf_records=c.fetchall()
    c.execute("SELECT * FROM hackerearth WHERE oid=" + record_id)
    earth_records=c.fetchall()
    c.execute("SELECT * FROM hackerrank WHERE oid=" + record_id)
    rank_records=c.fetchall()
    c.execute("SELECT * FROM cc WHERE oid=" + record_id)
    cc_records=c.fetchall()
    c.execute("SELECT * FROM atco WHERE oid=" + record_id)
    atco_records=c.fetchall()
    ...
    for record in cf_records:
        cf_id_upd.insert(0, record[0])
        cf_rat_upd.insert(0, record[1])
    for record in earth_records:
        earth_id_upd.insert(0, record[0])
        earth_rat_upd.insert(0, record[1])
    for record in rank_records:
        rank_id_upd.insert(0, record[0])
        rank_rat_upd.insert(0, record[1])
    for record in cc_records:
        cc_id_upd.insert(0, record[0])
        cc_rat_upd.insert(0, record[1])
    for record in atco_records:
        atco_id_upd.insert(0, record[0])
        atco_rat_upd.insert(0, record[1])
...
def add():
    conn = sqlite3.connect('code.db')
    c = conn.cursor()
    c.execute("INSERT INTO myself VALUES(:f_name, :l_name, :phnumber, :mail)",  
        {   'f_name': f_name.get(),
            'l_name': l_name.get(),
            'phnumber': phnumber.get(),   
            'mail': mail.get()
        })#Values are inserted in the table
    c.execute("INSERT INTO cf VALUES(:cf_id, :cf_rat)",
        {   'cf_id': cf_id.get(),
            'cf_rat': cf_rat.get()
        })
    c.execute("INSERT INTO hackerearth VALUES(:earth_id, :earth_rat)", 
        {   'earth_id': earth_id.get(),
            'earth_rat': earth_rat.get()
        })
    c.execute("INSERT INTO hackerrank VALUES(:rank_id, :rank_rat)", 
        {   'rank_id': rank_id.get(),
            'rank_rat': rank_rat.get()
        })
    c.execute("INSERT INTO cc VALUES(:cc_id, :cc_rat)", 
        {   'cc_id': cc_id.get(),
            'cc_rat': cc_rat.get()
        })
    c.execute("INSERT INTO atco VALUES(:atco_id, :atco_rat)",
        {   'atco_id': atco_id.get(),
...
def query():
    ...
    #Query the database
    c.execute("SELECT oid,* FROM myself")#Selects all data from myself
    records=c.fetchall()
    c.execute("SELECT * FROM cf")
    cf_records=c.fetchall()
    c.execute("SELECT * FROM hackerearth")
    earth_records=c.fetchall()
    c.execute("SELECT * FROM hackerrank")
    rank_records=c.fetchall()
    c.execute("SELECT * FROM cc")
    cc_records=c.fetchall()
    c.execute("SELECT * FROM atco")
    atco_records=c.fetchall()
    ...
    for index,record in enumerate(cf_records):
        ...
        for i in record:
            if i=='':
                lookup_label=Label(ad, text="NULL")#If user has entered then will store NULL value
                lookup_label.grid(row=index+1,column=ew,pady=(10,0))
            else :
                lookup_label=Label(ad, text=i)
                lookup_label.grid(row=index+1,column=ew,pady=(10,0))
            ew+=1
    num=ew
    for index,record in enumerate(earth_records):
        num=ew
        for i in record:
            if i=='':
                lookup_label=Label(ad, text="NULL")#If user has entered then will store NULL value
    ...
    for index,record in enumerate(rank_records):
        ...
        for i in record:
            ...
            ew+=1
    num=ew
    for index,record in enumerate(cc_records):
        num=ew
        for i in record:
            if i=='':
                lookup_label=Label(ad, text="NULL")#If user has entered then will store NULL value
            ...
            num+=1
    ew=num
    for index,record in enumerate(atco_records):
        ew=num
        for i in record:
            if i=='':
                lookup_label=Label(ad, text="NULL")#If user has entered then will store NULL value
...
def search():
    ...
    def search_now():
        ...
        c.execute("SELECT oid,* FROM myself")
        records=c.fetchall()
        c.execute("SELECT oid,* FROM cf")
        cf_records=c.fetchall()
        c.execute("SELECT oid,* FROM hackerearth")
        earth_records=c.fetchall()
        c.execute("SELECT oid,* FROM hackerrank")
        rank_records=c.fetchall()
        c.execute("SELECT oid,* FROM cc")
        cc_records=c.fetchall()
        c.execute("SELECT oid,* FROM atco")
        atco_records=c.fetchall()
        s=0
        l=[]
                    ...
                    if record[0]==l[k]:
                        ...
                        for i in record:
                            if i==l[k]:
                                ...
                                if i=='':
                                    lookup_label=Label(md, text="NULL")
                                    lookup_label.grid(row=index+1,column=ew,pady=10)
                                else:
                                    lookup_label=Label(md, text=i)
                                    lookup_label.grid(row=index+1,column=ew,pady=10)
                            ew+=1
...
def delete():
    ...
    def delete_now():
        conn = sqlite3.connect('code.db')
        c = conn.cursor()
        c.execute("DELETE from myself WHERE oid=" + delete_box.get())# Compare delete id
        c.execute("DELETE from hackerrank WHERE oid=" + delete_box.get()) 
        c.execute("DELETE from hackerearth WHERE oid=" + delete_box.get())
        c.execute("DELETE from atco WHERE oid=" + delete_box.get())
        c.execute("DELETE from cf WHERE oid=" + delete_box.get())
        c.execute("DELETE from cc WHERE oid=" + delete_box.get())
